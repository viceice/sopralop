...
public class QuickHull3D {
     ...
     *                this array will be three times the the number of input
     *                points.
     ...
     * @throws IllegalArgumentException
     *                 the number of input points is less than four or greater
     *                 than 1/3 the length of <code>coords</code>, or the
     *                 points appear to be coincident, colinear, or coplanar.
     */
     ...
     * @throws IllegalArgumentException
     *                 the number of input points is less than four or greater
     *                 then the length of <code>points</code>, or the points
     ...
     * non-convex if the centroid of either adjoining face is more than <i>tol</i>
     * above the plane of the other face. Similarly, a point is considered
     * outside a face if its distance to that face's plane is more than 10 times
     * <i>tol</i>.
     * 
     * <p>
	...
	do {
		...
		if (oppFaceDistance(hedge) > -this.tolerance
			|| oppFaceDistance(hedge.opposite) > -this.tolerance)
		    merge = true;
	    } else // wrt to the larger face; otherwise, just mark
	...
	for (int i = 0; i < 3; i++) {
	    double diff = this.maxVtxs[i].pnt.get(i)
		    - this.minVtxs[i].pnt.get(i);
	    if (diff > max) {
		max = diff;
		imax = i;
	    }
	...
	for (int i = 0; i < this.numPoints; i++) {
	    ...
	    if (dist > maxDist && this.pointBuffer[i] != vtx[0]
		    && // paranoid
		    this.pointBuffer[i] != vtx[1]
	...
	for (Vertex vtx = vtxNext; vtx != null; vtx = vtxNext) {
		...
		if (this.debug && vtx.index == this.findIndex)
		    System.out.println(this.findIndex + " CLAIMED BY "
